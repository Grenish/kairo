    ...
    <div className="min-h-svh h-full flex flex-col items-center justify-center py-20 px-4">
      ...
      <div className="relative w-full max-w-8xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 max-h-[800px] overflow-hidden md:max-h-none md:overflow-visible">
          {t.reviews.map(
            ({ id, profile, name, initials, source, testimonial }) => (
              <TestimonialCard
                key={id}
                profile={profile}
                name={name}
        ...
        <div className="absolute bottom-0 left-0 w-full h-24 bg-linear-to-t from-background to-transparent md:hidden" />
      </div>
      <h2 className="text-xl md:text-2xl mt-10 font-semibold text-center">
        {t.subtitle}
      </h2>
    </div>
  );
